...
export async function readGarminDatabase() {
  if (process.env.YULIANG_DATA_SOURCE !== "garmin-docker") throw new Error("data_not_configured");
  const stdout = await new Promise((resolve, reject) => {
    const child = spawn("docker", ["exec", "-i", "garmin-local-influxdb-1", "sh", "-c", reader], { stdio: ["pipe", "pipe", "pipe"] });
    let output = "";
    let overflow = false;
