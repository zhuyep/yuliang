...
import { isGarminDatabaseConfigured, readGarminDatabase } from "./garmin-database.mjs";
...
export function createAPI({ provider = readGarminDatabase, store = createFeedbackStore(), settings = modelSettings(), now = () => new Date() } = {}) {
  ...
  return async function handle(request, response, pathname) {
    ...
    if (pathname === "/api/session" && request.method === "GET") {
      send(response, 200, { token, dataConfigured: isGarminDatabaseConfigured(), model: { enabled: settings.enabled, sensitiveAllowed: settings.allowSensitive } }); return true;
    }
    ...
    try {
      ...
      send(response, dataFailure || storageFailure ? 503 : 400, { error: dataFailure
        ? "无法读取本机可穿戴数据源或记录有歧义。请检查适配器与数据状态；没有用示例数据冒充真实数据。"
